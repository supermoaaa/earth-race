...
#Generate an orientation with one definite (locked) axis and another axis aligned to a vector as close as possible without breaking the locked axis. Default Z locked with Y tracking.
def vectrack(v0, v1, lock_axis = 2, track_axis = 1):
	...
	lock = v0.normalized()
	track = v1.normalized()
	if abs(lock.dot(track)) == 1: #Gimbal lock! (Vectors are either identical or the inverse of each other)
	   return Matrix([[1,0,0],[0,1,0],[0,0,1]])
	
	other = track.cross(lock).normalized()
	track = lock.cross(other)
	
	other_axis = 3-(lock_axis+track_axis) #No idea where I pulled this operation from.
	
	if not (lock_axis-1)%3==track_axis: #Or this one. I must have hammerspace!
		other = -other
...
def towards(current, target, amount):
	if current > target: return current - min(abs(target-current), amount)
	if current < target: return current + min(abs(target-current), amount)
	return current

def sign(the_number):
	if the_number<0: return -1.0
	elif the_number>0: return 1.0
	raise ValueError
...
def dualplanelimit(pos, dir, radius):
	diff = dir.dot(pos)
	if diff>+radius: pos -= dir*(diff-radius)
	if diff<-radius: pos -= dir*(radius+diff)
	return pos
