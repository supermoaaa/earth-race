...
class r_wheel:
	...
	def step(self, dt):
		...
		if self.ray[0]:
			...
			try:
				...
				log('error','Fnormal = max(s_spring_force, 0) avec s_spring_force='+str(s_spring_force))
	...
	def getGroundCoefFriction(self, ray):
		if not ray is None and len(ray)>=4 and not ray[3] is None and ray[3].material_name in gl.matFriction:
