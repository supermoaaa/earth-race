...
class vehicleSimulation(object):
	def __init__( self, vehicle_type, wheel_type, keys ):
		gear=[[]]
		boostPower=int()
		print('vehicle init')
		print(vehicle_type)
		...
		print(keys)
		try:
			gl.LibLoad(gl.expandPath("//")+"objects/vehicles/"+vehicle_type+".blend","Scene")
			print(gl.getCurrentScene().objects)
		except:
			print("impossible d'ouvrir le fichier "+gl.expandPath("//")+"objects/vehicles/"+vehicle_type+".blend")
			print("heu… ben ça vas faire un bordel monstre")
		propertieFile = open(gl.expandPath("//")+"objects/vehicles/"+vehicle_type+".cfg", "r")
		for line in propertieFile:
			if "bloc = " in line:
				self.addPiece(line[7:])
			if "gear = " in line:
				self['gear']=eval(line[7:])
			if "boostPower = " in line:
				self['boostPower']=int(line[13:])
	...
	def addPiece( self, piece ):
		...
		try:
			child = scene.addObject( piece, own, 1 )
			try:
			...
			print("objet \""+piece+"\" non trouvé")

	def simulate( self ):
		cont = gl.getCurrentController()
		
		#print(dir(gl.getCurrentScene()))
